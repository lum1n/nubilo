...
func (c *Client) EnsureCollection(kind, name string) (*syncengine.Collection, error) {
	var col syncengine.Collection
	err := c.doJSON(http.MethodPost, "/sync/v1/collection", map[string]any{"kind": kind, "name": name}, &col)
	if err != nil {
		return nil, err
	}
	return &col, nil
